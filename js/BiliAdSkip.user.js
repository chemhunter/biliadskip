...
(function() {
	...
	const CONFIG_SCHEMA = {
		...
		// 以下为非UI配置项
		CLOUD_PLATFORM_SERVICE: { default: 'supabase', label: '云端公益AI平台（默认即可）', type: 'select', hidden: true,
								 options: [{ value: 'supabase', text: 'supabase' }, { value: 'vercel', text: 'vercel' }]},
		ANALYZE_DNAMAKU:        { default: "", label: '指定弹幕文本统计', type: 'text', hidden: true },
     ...
     * @param {boolean} showFeedback 是否显示点击反馈（默认true）
     * @returns {HTMLButtonElement}
     */
	function createButton(text, onClick, extraStyles = '', showFeedback = true) {
		const button = document.createElement('button');
		button.textContent = text;
		button.style.cssText = `padding: 5px 10px; border: 1px solid #ccc; background: #f0f0f0; border-radius: 4px; cursor: pointer; font-size: 15px; ${extraStyles}`;
		if (onClick) {
	...
	async function uploadAdTimeDataToCloud(bv, timestamp_range, source, NoAD = null) {
		...
		const TIMEOUT_MS = 15000; // 超时时间（毫秒）
		let lastError = null;
		...
		try {
			...
			lastError = err;
			console.error(`❌ 调用上传接口失败:`, err.name === 'AbortError' ? `请求超时 (${TIMEOUT_MS/1000}s)` : err.message);
		}
    ...
    * @returns {{hasAd: boolean, commentText: string, goods: string, reason: string|null}|null}  */
	...
	async function sendSubtitlesToAI(bvNumber, subtitles) {
		...
		const provider= configLibrary.lastSelected || 'kimi';
		const currentConfig = configLibrary[provider] || {};

		const apiUrl = currentConfig.apiUrl;
		const aiModel = currentConfig.model;
		const apiKey = currentConfig.apiKey || await GM_getValue(`apiKey_${provider}`, null);
		...
		// 3. 构造请求
		log(`供应商：${provider} ，模型：${currentConfig.model}`)
...
`;
		...
		if (isGemini) {
			...
			body = {
				...
				thinking: { "type": isThinkingModel ? "enabled" : "disabled", "thinking_budget": 500 },
		...
		try {
			...
			if (!response.ok) {
				...
				if (errorMessage.includes('inappropriate content')) {
					debuglog(`🚫 ${provider} 的模型：${currentConfig.model}  因“内容不适宜”拒绝了请求 [${bvNumber}]`);
					return null;
				}
			...
			console.error(`❌[${bvNumber}] 本地请求AI失败，${provider} ，模型：${currentConfig.model}`, error.message);
		}
	}
	...
	// AI配置数据
	const aiOptions = [
		{value: 'aliyun', text: '阿里云-建议', apiUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions',
		 model: `qwen3.8-max, deepseek-v4-flash-0731,qwen3.7-flash-2026-07-15, qwen3.7-flash, glm-5.2, kimi-k2.7-code,
		 qwen3.7-max-2026-06-08, qwen3.7-plus, qwen3.7-plus-2026-05-26, qwen3.7-max-2026-05-17, qwen3.7-max-preview,
		 qwen3.7-max, qwen3.7-max-2026-05-20, deepseek-v4-pro, deepseek-v4-flash, kimi-k2.6, glm-5.1,
		 tongyi-xiaomi-analysis-pro, tongyi-xiaomi-analysis-flash,`
		},
		{ value: 'deepseek', text: 'DeepSeek-建议', apiUrl: 'https://api.deepseek.com/v1/chat/completions', model: 'deepseek-v4-pro, deepseek-v4-flash' },
		{value: 'kimi', text: 'Kimi', apiUrl: 'https://api.moonshot.cn/v1/chat/completions', model: 'kimi-k3, kimi-k2.7-code, kimi-k2.6, kimi-k2.5, moonshot-v1-32k' },
		{value: 'glm', text: 'GLM', apiUrl: 'https://open.bigmodel.cn/api/paas/v4/chat/completions', model: 'glm-5.2, glm-5.1, glm-5, glm-4.7, glm-4.7-flash, glm-4.6, glm-4.5, glm-4.5-air' },
		{value: 'baidu', text: '百度千帆', apiUrl: 'https://qianfan.baidubce.com/v2/chat/completions', model: 'ernie-5.1, ernie-4.5-turbo-32k, ernie-x1.1, deepseek-v4-flash, qwen3.5-397b-a17b, qwen3.5-122b-a10b, qwen3.5-27b'},
		{value: 'siliconflow', text: '硅基流动', apiUrl: 'https://api.siliconflow.cn/v1/chat/completions', model: 'deepseek-ai/DeepSeek-V4-Pro, deepseek-ai/DeepSeek-V4-Flash, MiniMaxAI/MiniMax-M2.5' },
		{value: 'ChatGPT', text: 'OpenAI', apiUrl: 'https://api.openai.com/v1/chat/completions', model: 'gpt-5.6-sol, gpt-5.6-terra, gpt-5.6-luna, gpt-5.5, gpt-5.5-pro, gpt-5.4, gpt-5.4-pro, gpt-5.4-mini, gpt-5.4-nano' },
		{ value: 'custom1', text: '自定义AI-1', apiUrl: '', model: '' },
		{ value: 'custom2', text: '自定义AI-2', apiUrl: '', model: '' },
	];

	/** 创建AI配置面板内容（不包含窗口容器） */
	function buildAiConfigPanel() {
		const panel = document.createElement('div');
		panel.style.cssText = 'padding: 10px;';
		...
		// 定义链接数据：文本、URL、背景色、悬停提示
		const linkData = [
			{
				text: '阿里云',
				url: 'https://bailian.console.aliyun.com/?tab=model#/api-key',
				bgColor: '#FF6A00',          // 橙色
				tooltip: '阿里云百炼平台，上百个模型，每个模型1M免费tokens，3个月内有效，推荐用这个'
			},
			{
				text: 'Deepseek',
				url: 'https://platform.deepseek.com/',
				bgColor: '#4D6BFE',          // 蓝色
				tooltip: 'Deepseek 平台，v4-flash模型性价比高'
			},
			{
				text: 'Kimi',
				url: 'https://platform.moonshot.cn/console/api-keys/',
				bgColor: '#A855F7',          // 紫色
				tooltip: 'Kimi 平台，新用户赠送 15元代金券'
			},
			{
				text: 'GLM',
				url: 'https://bigmodel.cn/usercenter/proj-mgmt/apikeys',
				bgColor: '#22C55E',          // 绿色
				tooltip: '智谱AI，新用户赠送 2000万 Token，永久有效'
			},
			{
				text: '硅基流动',
				url: 'https://cloud.siliconflow.cn/sft-keejoek1ys/account/ak',
				bgColor: '#3B82F6',          // 亮蓝色
				tooltip: '新用户赠送 2000万 Token（价值约14元）'
			}
		];

		// 创建容器
		const linksContainer = document.createElement('div');
		linksContainer.style.cssText = 'margin: 40px 0; text-align: center;';

		// 添加描述文本
		const descriptionText = document.createTextNode('获取apikey：');
		linksContainer.appendChild(descriptionText);

		// 循环创建按钮样式的链接
		linkData.forEach(({ text, url, bgColor, tooltip }) => {
			const link = document.createElement('a');
			link.href = url;
			link.textContent = text;
			link.target = '_blank';
			link.title = tooltip; // 鼠标悬停提示

			// 按钮样式（白字、圆角、内边距、背景色）
			link.style.cssText =`display: inline-block; padding: 6px 14px; margin: 0 5px; border-radius: 6px; color: #ffffff; text-decoration: none; font-size: 14px; transition: opacity 0.2s;`;
		...
		tip.textContent = '⚠️：apikey 请保管好，勿告诉别人，否则其他人会盗用你的额度';
		tip.style.cssText = 'color: red; margin-top: 20px; font-size: 15px;';
		linksContainer.appendChild(tip);

		// 将容器添加到 panel（假设 panel 已定义）
		...
		const saveButton = createButton('保存配置', async (button) => {
	...
	/** 创建时间戳配置面板内容（不包含窗口容器） */
	async function buildTimestampPanel() {
		const panel = document.createElement('div');
		panel.style.cssText = 'padding: 10px;';
...
`;
		...
		let noAdButton;
		if (isVideoPage) {
			noAdButton = createButton('', async (button) => {
				const currentBv = bvInput.value.trim();
				const freshData = await getStoredAdTime(currentBv);
				...
				if (noAd) {
					...
					// 同步更新内存缓存
					const noAdIndex = scriptCache.noAdDbKeys.indexOf(currentBv);
					if (noAdIndex > -1) scriptCache.noAdDbKeys.splice(noAdIndex, 1);
					// 同步更新跨域缓存
					const crossDomainCache = await GM_getValue('biliCrossDomainCache', { mainAdDbKeys: [], noAdDbKeys: [] });
					const cacheNoAdIndex = crossDomainCache.noAdDbKeys.indexOf(currentBv);
					if (cacheNoAdIndex > -1) crossDomainCache.noAdDbKeys.splice(cacheNoAdIndex, 1);
					await GM_setValue('biliCrossDomainCache', crossDomainCache);
					// 立即更新按钮状态
					button.textContent = '标记该页无广告';
					button.style.backgroundColor = '#27ae60';
					await uploadAdTimeDataToCloud(currentBv, null, 'manual', false);
				} else {
					// 标记无广告
					currentData.noAd = true;
					await GM_setValue(currentBv, currentData);
					await markVideoAsNoAd(currentBv, { reason: 'manual' });
					// 立即更新按钮状态
					button.textContent = '撤销该页无广告';
					button.style.backgroundColor = '#f39c12';
				}
				await refreshManualUI();
			}, '', false);
			const isNoAd = storedData === 'noAd';
			noAdButton.textContent = isNoAd ? '撤销该页无广告' : '标记该页无广告';
			noAdButton.style.backgroundColor = isNoAd ? '#f39c12' : '#27ae60';
			noAdButton.style.color = 'white';
			buttonContainer.appendChild(noAdButton);
		}

		// ----- 保存按钮 -----
		const saveButton = createButton('保存时间戳', async (button) => {
			const currentBv = bvInput.value.trim();
			...
			await monitorTimestamp(currentBv, dataTimestamp, 'manual', { uploadCloud: true, saveTimestamp: true });
			await refreshManualUI();
		});
		saveButton.style.backgroundColor = '#129cf3';
		buttonContainer.appendChild(saveButton);

		// ----- 删除按钮 -----
		const deleteButton = createButton('删除记录', async (button) => {
			const currentBv = bvInput.value.trim();
			if (!currentBv) {
		...
		deleteButton.id = 'bili-manual-delete-btn';
	...
	/** 创建白名单管理面板内容（不包含窗口容器） */
	async function buildWhitelistPanel() {
		const panel = document.createElement('div');
		panel.style.cssText = 'padding: 10px;';
		...
		const toggleButton = createButton('执行', handleToggle, '', false);
		...
		if (currentUpInfo && currentUpInfo.name) {
			...
			panel.appendChild(currentUserRow);
		}
	...
	/** 创建脚本高级配置面板内容（不包含窗口容器） */
	function buildScriptSettingsPanel() {
		const panel = document.createElement('div');
		panel.style.cssText = 'padding: 10px;';
		...
		configItems.forEach((item, index) => {
			const row = document.createElement('div');
			row.style.cssText = 'display: flex; align-items: center; justify-content: space-between; min-height: 28px;';
			...
			numberSpan.style.cssText = 'color: #999; font-weight: bold; min-width: 20px; font-size: 13px;';
			labelContainer.appendChild(numberSpan);

			const label = document.createElement('label');
			label.textContent = item.label;
			label.style.cssText = 'color: #555; font-weight: 500; font-size: 13px;';
			...
			if (item.type === 'checkbox') {
				...
				input.style.cssText = 'width: 168px; padding: 2px 6px; border: 1px solid #ddd; border-radius: 4px; outline: none; background: #fff; cursor: pointer; transition: border-color 0.2s; font-size: 13px;';
				...
				input.style.cssText = 'width: 150px; padding: 2px 6px; border: 1px solid #ddd; border-radius: 4px; outline: none; transition: border-color 0.2s; font-size: 13px;';
				input.onfocus = () => input.style.borderColor = '#00a1d6';
				input.onblur = () => input.style.borderColor = '#ddd';
			}
		...
		btnContainer.style.cssText = 'margin-top: 20px; display: flex; justify-content: center; gap: 10px;';

		const saveBtn = createButton('保存并应用', async (button) => {
	...
	/** 打开统一的标签页设置界面 */
	async function openUnifiedSettingsPopup(initialTab = 'ai') {
		if (settingsContainer && settingsContainer.isConnected) {
		...
		container.id = 'bili-ad-skipper-unified-settings';
		applyPopupStyle(container, { width: '720px' });
		settingsContainer = container;
		addCloseX(container);

		const title = document.createElement('h3');
		title.textContent = 'biliadskip设置';
		title.style.cssText = 'text-align: center; margin-bottom: 20px; font-weight: bold;';
		container.appendChild(title);
		draggableManager.makeDraggable(container, title);

		const body = document.createElement('div');
		body.style.cssText = 'display: flex; gap: 20px; min-height: 380px;';

		const tabs = document.createElement('div');
		tabs.style.cssText = 'width: 100px; flex-shrink: 0; border-right: 1px solid #e8e8e8; display: flex; flex-direction: column; gap: 4px; padding-right: 10px;';

		const tabStyles = {
			base: 'padding: 8px 12px; border: none; background: transparent; cursor: pointer; text-align: left; border-radius: 4px; transition: all 0.2s;',
			active: 'background: #e6f7ff; color: #00aeec; font-weight: bold;',
			inactive: 'color: #333;'
		};

		const tabAI = document.createElement('button');
		tabAI.textContent = 'AI配置';
		tabAI.style.cssText = tabStyles.base + tabStyles.inactive;
		tabAI.onclick = () => switchTab('ai');

		const tabTimestamp = document.createElement('button');
		tabTimestamp.textContent = '时间戳';
		tabTimestamp.style.cssText = tabStyles.base + tabStyles.inactive;
		tabTimestamp.onclick = () => switchTab('timestamp');

		const tabWhitelist = document.createElement('button');
		tabWhitelist.textContent = '白名单';
		tabWhitelist.style.cssText = tabStyles.base + tabStyles.inactive;
		tabWhitelist.onclick = () => switchTab('whitelist');

		const tabSettings = document.createElement('button');
		tabSettings.textContent = '高级';
		tabSettings.style.cssText = tabStyles.base + tabStyles.inactive;
		tabSettings.onclick = () => switchTab('settings');

		tabs.appendChild(tabAI);
		tabs.appendChild(tabTimestamp);
		...
		const aiPanel = buildAiConfigPanel();
		const timestampPanel = await buildTimestampPanel();
		const whitelistPanel = await buildWhitelistPanel();
		const settingsPanel = buildScriptSettingsPanel();
		...
		content.appendChild(timestampPanel);
		content.appendChild(whitelistPanel);
		...
		function switchTab(tab) {
			...
			[tabAI, tabTimestamp, tabWhitelist, tabSettings].forEach(el => {
			...
			if (tab === 'ai') {
				tabAI.style.background = '#e6f7ff';
				tabAI.style.color = '#00aeec';
				tabAI.style.fontWeight = 'bold';
				aiPanel.style.display = 'block';
			} else if (tab === 'timestamp') {
				tabTimestamp.style.background = '#e6f7ff';
				tabTimestamp.style.color = '#00aeec';
				tabTimestamp.style.fontWeight = 'bold';
				timestampPanel.style.display = 'block';
			} else if (tab === 'whitelist') {
				tabWhitelist.style.background = '#e6f7ff';
				tabWhitelist.style.color = '#00aeec';
				tabWhitelist.style.fontWeight = 'bold';
				whitelistPanel.style.display = 'block';
			} else if (tab === 'settings') {
				tabSettings.style.background = '#e6f7ff';
				tabSettings.style.color = '#00aeec';
	...
	async function addToolbarButton() {
		...
		for (let i = 0; i < 30; i++) {
			toolbarRight = document.querySelector('.video-toolbar-right');
			if (toolbarRight) break;
			await new Promise(resolve => setTimeout(resolve, 1000));
		}

		if (!toolbarRight) return;
		if (document.getElementById('biliadskip-wrapper')) return;
		...
		const parent = toolbarRight.parentNode;
		if (!parent) return;

		// 1. 创建自适应宽度的包裹容器 (根据父容器自动调整，内部元素居右)
		const wrapper = document.createElement('div');
		wrapper.id = 'biliadskip-wrapper';
		wrapper.style.cssText = `flex: 1; display: flex; justify-content: flex-end; align-items: center; min-width: 0;`;
		wrapper.setAttribute('data-v-7f62e6b7', '');
		...
		line-height: 1;
    `;
		button.setAttribute('data-v-34aec60a', '');
		button.setAttribute('data-v-7f62e6b7', '');
    ...
    `;
		...
		text.textContent = 'biliadskip';
		text.style.cssText = 'font-weight: 500;';

		button.appendChild(icon);
		button.appendChild(text);
		button.onclick = () => openUnifiedSettingsPopup('ai');

		// 3. 将按钮放入容器，容器插入到工具栏右按钮前面
		wrapper.appendChild(button);
		parent.insertBefore(wrapper, toolbarRight);
	}
	...
	// 使用统一的标签页设置界面
	registerMenuUI("🤖AI配置", 'bili-ad-skipper-unified-settings-ai', () => openUnifiedSettingsPopup('ai'));
	registerMenuUI("⌚时间戳配置", 'bili-ad-skipper-unified-settings-timestamp', () => openUnifiedSettingsPopup('timestamp'));
	registerMenuUI("📋白名单管理", 'bili-ad-skipper-unified-settings-whitelist', () => openUnifiedSettingsPopup('whitelist'));
	registerMenuUI("⚙️脚本设置", 'bili-ad-skipper-unified-settings-script', () => openUnifiedSettingsPopup('settings'));
